...
const operationalExitCriteria = document.querySelector("#operational-exit-criteria");
...
function renderMetrics(summary) {
  const cards = [
    ...
    ["exit_criteria_gap_items", "Exit criteria", "danger"],
  ];
...
function renderOwnerHandoff(page) {
  const rows = page.items || [];
  ownerHandoff.innerHTML = rows.length
    ? rows
        .map(
          (item) =>
            `<tr><td>${escapeHtml(item.issue_type)}</td><td>${escapeHtml(item.owner || "-")}</td><td>${escapeHtml(item.application_name)}</td><td>${escapeHtml(item.latest_scan_status || "missing")}</td><td>${escapeHtml(item.detail)}</td></tr>`
        )
        .join("")
    : `<tr><td colspan="5">No owner handoff gaps</td></tr>`;
}

function renderRepositoryInventoryAssurance(page) {
  const rows = page.items || [];
  repositoryInventoryAssurance.innerHTML = rows.length
    ? rows
        .map(
          (item) =>
            `<tr><td>${escapeHtml(item.gap_type)}</td><td>${escapeHtml(item.repository_name ? `${item.repository_owner}/${item.repository_name}` : "-")}</td><td>${escapeHtml(item.provider || "-")}</td><td>${escapeHtml(formatDateTime(item.last_synced_at))}</td><td>${escapeHtml(item.detail)}</td></tr>`
        )
        .join("")
...
function renderCriticalHighTriage(page) {
  const rows = page.items || [];
  criticalHighTriage.innerHTML = rows.length
    ? rows
        .map(
          (item) =>
            `<tr><td>${escapeHtml(item.severity)}</td><td>${escapeHtml(item.status)}</td><td>${escapeHtml(item.application_name)}</td><td>${escapeHtml((item.missing || []).join(", ") || "-")}</td><td>${escapeHtml(item.detail)}</td></tr>`
        )
        .join("")
...
function renderOperationalExitCriteria(page) {
  const rows = page.items || [];
  operationalExitCriteria.innerHTML = rows.length
    ? rows
        .map(
          (item) =>
            `<tr><td>${escapeHtml(item.check)}</td><td>${escapeHtml(item.status)}</td><td>${escapeHtml(item.count)}</td><td>${escapeHtml(item.target ?? "-")}</td><td>${escapeHtml(item.detail)}</td></tr>`
        )
        .join("")
    : `<tr><td colspan="5">No operational exit criteria</td></tr>`;
}
...
async function refresh() {
  ...
  operationalExitCriteria.innerHTML = `<tr><td colspan="5">Loading</td></tr>`;
  try {
    const [
      summary,
      criticalFindings,
      ...
      loadJson("/operations/exit-criteria?status=warn&limit=10"),
    ]);
    ...
    renderOperationalExitCriteria(operationalExitCriteriaPage);
  } catch (error) {
    ...
    operationalExitCriteria.innerHTML = `<tr><td colspan="5">Unable to load operational exit criteria</td></tr>`;
  }
}
